  ...
  useLoginCheck(login)
    return(<LayOut login={login}>
      <div className="flex mx-14 my-10">
       <nav className="border-r-2 border-gray-300 w-80 h-[75vh] pr-5">
       <div className={`bg-gray-100 w-72 ${state.profilState?"h-80":"h-96"} rounded-xl pt-2 relative shadow-xl`}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" 
        strokeWidth={1.5} stroke="currentColor" 
        className="w-28 aspect-square bg-white rounded-full mt-8 mx-auto p-1">
  <path strokeLinecap="round" strokeLinejoin="round" 
  d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
</svg>

<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" 
className="w-6 aspect-square bg-white absolute right-[6rem] rounded-full border-2 border-black top-[7.8rem]">
...
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" 
className="w-6 h-6 bg-white absolute left-[6rem] rounded-full border-2 border-black top-[7.8rem]">
...
className="ml-8 mt-3 w-56 rounded-lg p-1 font-bold border-2 text-sm border-blue-500 outline-none "/>}
...
className={`w-6 h-6 bg-white border-2 border-black rounded-full absolute ${state.profilState?"bottom-7":"bottom-3"} right-5 cursor-pointer`}>
  <path strokeLinecap="round" strokeLinejoin="round" d="M10.343 3.94c.09-.542.56-.94 1.11-.94h1.093c.55 0 1.02.398 1.11.94l.149.894c.07.424.384.764.78.93.398.164.855.142 1.205-.108l.737-.527a1.125 1.125 0 011.45.12l.773.774c.39.389.44 1.002.12 1.45l-.527.737c-.25.35-.272.806-.107 1.204.165.397.505.71.93.78l.893.15c.543.09.94.56.94 1.109v1.094c0 .55-.397 1.02-.94 1.11l-.893.149c-.425.07-.765.383-.93.78-.165.398-.143.854.107 1.204l.527.738c.32.447.269 1.06-.12 1.45l-.774.773a1.125 1.125 0 01-1.449.12l-.738-.527c-.35-.25-.806-.272-1.203-.107-.397.165-.71.505-.781.929l-.149.894c-.09.542-.56.94-1.11.94h-1.094c-.55 0-1.019-.398-1.11-.94l-.148-.894c-.071-.424-.384-.764-.781-.93-.398-.164-.854-.142-1.204.108l-.738.527c-.447.32-1.06.269-1.45-.12l-.773-.774a1.125 1.125 0 01-.12-1.45l.527-.737c.25-.35.273-.806.108-1.204-.165-.397-.505-.71-.93-.78l-.894-.15c-.542-.09-.94-.56-.94-1.109v-1.094c0-.55.398-1.02.94-1.11l.894-.149c.424-.07.765-.383.93-.78.165-.398.143-.854-.107-1.204l-.527-.738a1.125 1.125 0 01.12-1.45l.773-.773a1.125 1.125 0 011.45-.12l.737.527c.35.25.807.272 1.204.107.397-.165.71-.505.78-.929l.15-.894z" />
  <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
</svg>
</div> 

 <div className="font-bold text-center w-72 mt-7">
...
<aside className="overflow-x-hidden pt-3 w-[80%]">
    <div className="flex font-bold justify-between px-8 w-full text-lg ml-9 border-b-2 border-gray-300 pb-3">
    ...
    <form onSubmit={e=>e.preventDefault()} className="relative">
        <input type="text" className="w-[90%] shadow-lg p-2 font-bold bg-gray-100 mt-5 ml-9 rounded-lg outline-none"/>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={3} 
   ...
   
{
  ...
  {Array(5).fill({avatar:"",name:"follower",
   ...
   <div key={index} className="my-5 ml-9 bg-gray-100 rounded-xl flex h-32 w-[90%] p-4 shadow-lg">
       {item.avatar?
...
<div className=" pt-3 text-lg max-w-xs">
<div className="font-bold w-full overflow-hidden text-ellipsis whitespace-nowrap">{item.name}</div>
...
${item.follow?"border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white":"border-red-500 text-red-500 hover:bg-red-500 hover:text-white"}`}/>
  ...
  :
  <div className="max-h-[60vh] overflow-auto mt-5 scrollbar-hide">
  {Array(3).fill({avatar:"",name:"request",
   timeLine:"Hi, i'm Devops engineer request.",follow:true}).map((item,index)=>{return(
   <div key={index} className="my-5 ml-9 bg-gray-100 rounded-xl flex h-32 w-[90%] p-4 shadow-lg">
       {item.avatar?
       <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" 
       className="w-20 aspect-square p-1 bg-white rounded-full my-auto mr-4">
...
<div className="border-l-4 border-gray-400 pl-5 flex w-full justify-between relative">
<div className="block pt-3 text-lg max-w-xs">
<div className="font-bold w-full overflow-hidden text-ellipsis whitespace-nowrap">{item.name}</div>
<div className="pt-4 font-bold w-full overflow-hidden text-ellipsis whitespace-nowrap">{item.timeLine}</div>
</div>
<input type="button" value={item.follow?"follow":"unfollow"} 
className={`border-2 rounded-lg text-md h-12 px-2 font-bold relative top-5 right-4 bg-white cursor-pointer 
${item.follow?"border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white":"border-red-500 text-red-500 hover:bg-red-500 hover:text-white"}`}/>
</div>
   </div>)})}
  </div>
