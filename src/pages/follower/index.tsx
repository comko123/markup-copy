      ...
      <nav className="border-b-2 lg:border-r-2 lg:border-b-transparent border-gray-300 w-full lg:w-[32%] h-max lg:pr-5">
        <div className={`bg-gray-100 pt-7 lg:pt-0 w-full lg:w-[95%] ${profilState ? "h-52 lg:h-80 " : "pt-10 h-60 lg:h-[25rem]"} flex justify-center lg:flex-col rounded-xl px-2 relative shadow-xl`}>
          <div className="h-[35%] mt-4 lg:mt-0">
            {[{img:false,
            domain:userPhoto.default,
            position:"w-28 mx-auto p-1"},
            { outside:userPhoto.setting.outside,
              inside:userPhoto.setting.inside,
              position:"w-6 relative left-[5.5em] lg:left-[60%] -top-[2rem] border-2 border-black"},
          ...
          <div className={`flex flex-col ${profilState ? null : "-mt-6 lg:-ml-7 xl:ml-0"}`}>
            ...
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" onClick={() => setProfilState(state => { return !state })}
              ...
              className={`w-6 h-6 bg-white border-2 border-black rounded-full absolute ${profilState ? "bottom-2" : "bottom-3"} right-5 cursor-pointer`}>
