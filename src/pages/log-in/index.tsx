      ...
      <main className="flex flex-col justify-center items-center h-[70vh] md:h-[90vh] xl:h-[75vh] my-12 mx-10">
        ...
        >
          {["email", "password"].map(item => {
            return (
              <div className="font-bold flex flex-col" key={item}>
                <span className="ml-1 mb-1">{item}</span>
                <input
                  type={item !== "password" ? "email" : "password"}
                  className="border-2 border-blue-500 rounded-lg h-10 outline-none p-3 text-sm"
                />
              </div>
            )
          })}
          <input
            type="submit"
            value="Log In"
            className="bg-blue-500 p-2 mt-5 mb-2 rounded-lg font-bold text-lg text-white hover:opacity-70"
          />
        </form>
      </main>
    </Layout>
