  ...
  return (
    <>
      <header className="bg-blue-500 flex justify-between p-2 md:p-5 px-6 lg:px-24 font-bold fixed top-0 w-full z-10">
        <h3
          className={`text-md lg:text-2xl hover:text-white ${
          ...
          {login ? (
            <>
              {["Calender", "Chart", "Follower", "Log-Out"].map((item, index) => {
                return (
                  <span
                    key={index}
                    className={`${
                      pathname === `/${item.toLowerCase()}` ? "text-white" : null
                    } text-xs cursor-pointer lg:text-lg mx-1 lg:mx-3 hover:text-white`}
                    onClick={() => {
                      if (item === "Log-Out") {
              ...
              <svg
                ...
                className={`w-4 aspect-square lg:w-6 z-20 mx-1 lg:mx-3 ${
                  modalShowing ? "text-white" : "text-black"
              ...
              <AnimatePresence>
                {modalShowing ? (
                  <motion.div
                    ...
                    animate="display"
                    exit="end"
                    className="fixed right-[2%] md:right-[1%] lg:right-[2%] top-[2.2em] md:top-[2.3em] xl:top-[3.8em] origin-top w-[14rem] md:w-[16rem] h-[15.5rem] md:h-[18.5rem] my-2 bg-pink-100 rounded-xl"
                  >
                    <form onSubmit={e => e.preventDefault()} className="p-2">
                      <input
                        type="text"
                        placeholder="write to title"
                        className="outline-none p-2 w-full rounded-lg my-2 text-vxs md:text-sm"
                      />

                      <div className="flex flex-col mb-2">
                        <div className="flex justify-center">
                          {Object.keys(ctg).map(item => (
                            <div
                              key={item}
                              onClick={() =>
                                setModalState(state => {
                                  if (item === selectMainCtg)
                                    return {
                                      ...state,
                                      addToDoModalState: { ...addToDoModalState, selectMainCtg: "" }
                                    }
                                  else
                                    return {
                                      ...state,
                              ...
                              className={`${
                                selectMainCtg === item ? "bg-blue-500" : "bg-blue-300"
                              } hover:ring hover:ring-blue-500 hover:ring-offset-2 text-vxs md:text-sm m-1 p-1 text-white rounded-md`}
                            >
                              {item}
                            </div>
                          ))}
                        ...
                        <div className="flex justify-center empty:h-[1.8rem]">
                          {selectMainCtg &&
                            ctg[selectMainCtg].map(item => (
                              <div
                                key={item}
                                onClick={() =>
                                  setModalState(state => {
                                    const setting = (params: string) => {
                                      return {
                                        ...state,
                                        addToDoModalState: {
                                          ...addToDoModalState,
                                    ...
                                    if (item === selectSubCtg) return setting("")
                                    else return setting(item)
                                  })
                                }
                                className={`${
                                  selectSubCtg === item ? "bg-blue-500" : "bg-blue-300"
                                } hover:ring hover:ring-blue-500 hover:ring-offset-2 text-vxs md:text-sm m-1 p-1 text-white rounded-md`}
                              >
                                {item}
                              </div>
                      ...
                      <div className="mb-2 text-vxs md:text-sm">
                        <span className="ml-1">level : </span>
                        <select className="p-1 rounded-lg outline-none text-vxs md:text-sm">
                          {["High", "Medium", "Low"].map(item => (
                            <option key={item} className="font-bold">
                              {item}
                            </option>
                          ))}
                        </select>
                      </div>
                      <div className="mb-2 text-vxs md:text-sm ml-1">
                        {" "}
                        <span>start date : </span>
                        <input
                          type="date"
                          className="bg-white text-vxs md:text-sm p-1 rounded-lg outline-none"
                        />
                      </div>
                      <div className="mb-3 text-vxs md:text-sm ml-1">
                        {" "}
            ...
            <>
              {["Log-In", "Sign-Up"].map(item => (
                <div key={item}>
                  <div
                    onClick={() => replace(`/${item.toLowerCase()}`)}
                    className={`${pathname === `/${item.toLowerCase()}` ? "text-white" : null} 
  text-xs lg:text-lg mx-1 lg:mx-3 hover:text-white cursor-pointer`}
                  >
                    {item}
                  </div>
                </div>
        ...
        <footer className="relative">
          <svg
            ...
            onClick={() =>
              setModalState(() => {
                return {
                  addToDoModalState: { ...addToDoModalState, modalShowing: false },
                  alertState: { alertShowing: !alertShowing }
                }
              })
            }
            className={`w-[2.5rem] aspect-square lg:w-[4%] ${
              alertShowing ? "text-white" : "text-black"
            } fixed bottom-[2em] right-[3em] hover:text-white cursor-pointer bg-blue-500 p-1 rounded-full shadow-md shadow-slate-400`}
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
                ...
                {alertSample.length ? (
                  <>
                    <div className="text-slate-600 flex text-vxs font-bold w-full justify-end border-b-2 pb-1 border-sky-500">
                      <span onClick={() => setAlertSample([])} className="hover:text-amber-600">
                        모두 읽기
                      </span>
                    </div>
                    <div className="max-h-full overflow-auto scrollbar-hide">
                      {alertSample.map(item => (
                        <div
                          key={item.content}
                          className="flex w-full justify-between border-2 bg-white rounded-lg border-sky-500 my-2 p-2"
                        >
                          <div>
                            {item.avatar ? (
                              <svg
                          ...
                          <div className="flex flex-col text-vxs max-w-[60%] lg:max-w-full">
                            <div className="mx-1 overflow-hidden text-ellipsis whitespace-nowrap">
                              {item.content}
                            </div>
                          ...
                          <svg
                            ...
                            onClick={() =>
                              setAlertSample(state =>
                                state.filter(select => select.content !== item.content)
                              )
                            }
                          >
                            <path
                  ...
                  <div className="flex items-center justify-center w-full h-full text-vxs md:text-sm font-bold">
