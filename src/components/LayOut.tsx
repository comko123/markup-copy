...
<header className="bg-blue-500 flex justify-between p-2 lg:p-5 lg:px-24 font-bold">
 ...
 {login?<>
    ...
    return(
    <span key={index} 
    className={`${pathname===`/${item.toLowerCase()}`?"text-white" :null} text-xs lg:text-lg mx-1 lg:mx-3 hover:text-white`}
    onClick={e=>{
      if(e.currentTarget.textContent!=="Log-Out"){console.log("Log-Out")}
      if(e.currentTarget.innerText==="Chart")push('/chart')
  }}
    >{item}</span>)
  })}
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"
  onClick={()=>push('/profil')}
  className="w-4 aspect-square lg:w-6 mx-1 lg:mx-3 hover:text-white">
  <path strokeLinecap="round" strokeLinejoin="round" d="M12 20.25c4.97 0 9-3.694 9-8.25s-4.03-8.25-9-8.25S3 7.444 3 12c0 2.104.859 4.023 2.273 5.48.432.447.74 1.04.586 1.641a4.483 4.483 0 01-.923 1.785A5.969 5.969 0 006 21c1.282 0 2.47-.402 3.445-1.087.81.22 1.668.337 2.555.337z" />
</svg>
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" 
className="w-4 aspect-square lg:w-6 hover:text-white">
 ...
 {["Log-In","Sign-Up"].map(item=><div key={item}>
  ...
  text-xs lg:text-lg mx-1 lg:mx-3 hover:text-white`}>{item}</div> 
 </div>)}
 </>
 }
</div></header>
